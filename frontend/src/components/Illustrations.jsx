// SVG components for the app: hair texture, cut type silhouettes (3/4 face, no face), barber chair, logo placeholder.

export const ScissorsLogo = ({ size = 40, className = "" }) => (
  <div
    className={`rounded-md flex items-center justify-center ${className}`}
    style={{ width: size, height: size, backgroundColor: "#E5E7EB" }}
    aria-label="Logo salon par défaut"
  >
    <svg width={size * 0.55} height={size * 0.55} viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export const BarberChair = ({ color = "#9CA3AF", size = 90, label, count }) => {
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      {typeof count === "number" && (
        <div className="text-[10px] label-uppercase" style={{ color: '#4F46E5' }}>{count} en attente</div>
      )}
    </div>
  );
};
